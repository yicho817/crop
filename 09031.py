...
class ImageProcessingThread(QThread):
    ...
    def run(self):
        ...
        if self.mode == "Cropping":
            for image_path in self.process_task_crop():
...
class ImageCropApp(QMainWindow):
    ...
    def load_image_and_labels(self, image_path):
        ...
        label_path = os.path.join(self.image_folder_path, label_name)
        classes_path = os.path.join(self.image_folder_path, classes_name)
        # Read image
        ...
        if os.path.exists(label_path) and os.path.exists(classes_path):
            # read class name
